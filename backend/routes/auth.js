...
router.post("/add-income", async (req, res) => {
  try {
    const { incomeTitle, amount, date, category } = req.body;
    const normalizedTitle = incomeTitle?.trim();
    const normalizedAmount = Number(amount);

    if (!normalizedTitle || !Number.isFinite(normalizedAmount) || !date || !category) {
      return res.status(400).json({ message: "All fields are required." });
    }
...
router.post("/add-expense", async (req, res) => {
  try {
    const { expenseTitle, amount, date, category } = req.body;
    ...
    await Plan.updateMany({ type: "Budget", category }, { $inc: { saved: normalizedAmount } });
