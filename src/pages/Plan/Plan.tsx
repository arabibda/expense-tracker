...
export const Plan = () => {
    ...
    const loadPlans = async () => {
        try {
            const [goalsResponse, budgetsResponse] = await Promise.all([
                fetch("http://localhost:5000/api/plans?type=Goal"),
                fetch("http://localhost:5000/api/plans?type=Budget"),
            ])
    ...
    const deleteGoal = async () => {
        ...
        try {
            await fetch(`http://localhost:5000/api/plans/${activeGoal.id}`, { method: "DELETE" })
